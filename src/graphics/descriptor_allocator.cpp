...
void descriptor_allocator::free(D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handle) noexcept
{
    if (cpu_handle.ptr == 0 || type < 0 || type >= D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES)
    {
        return;
    }
    descriptor_heap& heap{heaps_[type]};
    if (!heap.ptr)
    {
        return;
    }
    const auto it{std::ranges::find_if(
        heap.handles,
        [cpu_handle](const std::optional<descriptor_handle>& handle)
        {
            return handle && handle->cpu.ptr == cpu_handle.ptr;
        }
    )};
    if (it != heap.handles.end())
    {
