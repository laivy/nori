...
result<void> renderer::initialize_imgui()
{
    ...
    info.SrvDescriptorFreeFn = [](ImGui_ImplDX12_InitInfo* init_info, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handle, D3D12_GPU_DESCRIPTOR_HANDLE gpu_handle)
    {
        auto self{static_cast<renderer*>(init_info->UserData)};
        self->descriptor_allocator_->free(
            descriptor_handle{
